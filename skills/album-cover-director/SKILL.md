...
9. Compare all candidates at 56 px, 128 px, 256 px, full size, grayscale, and blur. Use the bundled `scripts/cover-ops.py contact-sheet` when Pillow is available, then run `scripts/cover-ops.py preflight` and `compare`. Objective image checks are automated; readable text must be verified by a human because OCR is intentionally not configured. Score with `assets/scorecard.yaml` and `references/evaluation-delivery.md`. Use `scripts/action-router.py` to choose a bounded next action from a recorded failure code. Reject work that reads primarily as a poster, advertisement, editorial illustration, app tile, game splash screen, or generic AI concept image rather than a durable music-release identity.
...
13. Export a 3000 x 3000 PNG and JPG plus a 256 px thumbnail with `scripts/cover-ops.py export`. Create `handoff-manifest.json` with `scripts/handoff-manifest.py` for the selected candidate. If the required image library is unavailable, provide the exact export specification and do not claim delivery completion.
