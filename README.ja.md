...
- 4年代・120作品、日本・東アジア24作品以上を含む観察コーパス
- スタイル名ではなく「何が正方形を組織するか」を表す12パターン
- 色違いではなく構造が異なる3方向
- 文字生成が崩れたときに後組版へ切り替える独立ゲート
- 56px、256px、原寸、グレースケール、ぼかしの比較
- 1回1変数、最大2サイクルの修正
...
[research/corpus.yaml](research/corpus.yaml) にあるのは書誌情報、出典リンク、独自の観察結果だけです。第三者のジャケット画像は収録しません。色面比は目視推定で、確認できないデザイナー名は明示的に未確認としています。

研究の起点は、[Cooper Hewitt「Art of Noise」](https://www.cooperhewitt.org/exhibition/art-of-noise/)や[Blue Note Recordsの視覚資料](https://www.bluenote.com/blue-note-wall-art/)などの美術館・レーベル資料です。
