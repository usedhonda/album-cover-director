...
以下の4枚は、スキルが目指す「タイトルを画像にする」関係を示す独自生成作例です。リポジトリ作者が権利を持つキャラクター参照を使ったデモであり、必須の画風や第三者ジャケットの参照ではありません。

| タイトル画像の構造 | 何を示すか |
| --- | --- |
| ![巨大なSECRETARY CHIがオフィス世界になるタイトルマップ。](docs/examples/title-map-secretary-chi.png) | **タイトル＝地図。** 文字の内部、カウンター、通路そのものがオフィス世界になり、タイトルが画面の地理を決めます。 |
| ![ちーと巨大なSECRETARY CHIが一体になるヒーローワードマーク。](docs/examples/hero-wordmark-secretary-chi.png) | **ヒーロー・ワードマーク。** 主役人物と巨大タイトルが同じ奥行きと重なりで画面を支配します。 |
| ![社長室の朝と巨大な朝を描いたジャケット。](docs/examples/japanese-title-shachoshitsu-no-asa.png) | **日本語の階層。** 上段の「社長室の」に対して、朱い巨大な「朝」が日の出とともに主役になります。 |
| ![ツン顔のちーが赤ペンを突き出す高密度な文字画像。](docs/examples/tsundere-secretary-chi.png) | **キネティック・ワードマーク。** 赤ペン、ジェスチャー、オフィスの断片、文字の筆致が一つのアクションになります。 |

4枚とも、正確なタイトルは画像の中で同時に作られています。フォントの後入れ、描き直し、後組版、合成で直してはいけません。文字が崩れた候補は、タイトルと画像の関係をより具体化したプロンプトで再生成します。
