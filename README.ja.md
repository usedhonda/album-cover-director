...
実行量は quick=3枚、standard=6枚、deep=12枚。文字モードは auto、image-native、post-typeset、custom-wordmark です。評価に出す各候補は、正確なタイトルがジャケット画像と一体化した完成候補です。文字なし原版は制作途中のレイヤーに限り、候補として提示・採用しません。
...
custom-wordmark ではフォント名だけを指定しません。骨格、幅、太さ、カウンター、端部、リズム、加工原理を定義します。正確な綴り、字間、行間、ベースラインを満たさないAI生成文字は採用せず、設計済みのタイトルと画像の関係を正確な後組版または描き直しで再構成します。文字なし原版は制作途中に限り、候補として提示しません。
