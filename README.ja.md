...
$album-cover-director は、アルバム、EP、シングルのジャケットを構想から3000px納品までディレクションするCodex Skill / Pluginです。専門は、文字を画像の中で成立させることです。正確なタイトルを物質世界そのものにするか、人物と同じ画面階層に置くかを選び、曲の意味を3つの異なる画面構造へ変換して、GPT Image 2で生成・比較します。
...
| ![ALBUM COVER DIRECTORが配管と共鳴器からなる機械的な音響構造になる。](docs/examples/mechanized-title-system.png) | ![ALBUM COVER DIRECTORが回転するクラブ光の空間になる。](docs/examples/rotating-club-title-system.png) |
| **機械音響システム** — 配管、ばね、共鳴器、配線が高密度な題字の工作物を組み上げる。 | **回転する光のフィールド** — 投影文字、スモーク、小さなミラーボールがクラブ全体を題字の空間にする。 |
...
この8例は、同格の2系統を示します。**物質タイトル世界**は、航路、工作物、ゲーム、標本などが文字の骨格を作る系統。**キャラクター主導**は、入力されたartist systemや楽曲briefが人物を必要とするとき、人物の姿勢・行為・世界と題字を一つの構造にする系統です。どちらも、完成したシーンの上に普通の文字を置くことを認めません。

## 特徴

- 物質タイトル世界とキャラクター主導という、2つの画像内題字システム
...
実行量は quick=3枚、standard=6枚、deep=12枚。文字モードは auto、image-native、custom-wordmark です。題字システムは auto、material-world、character-led から選べます。評価に出す各候補は、正確なタイトルがジャケット画像と一体化した完成候補です。タイトルは画像生成と同時に成立させ、あとからフォント、描き直し、合成で足しません。
